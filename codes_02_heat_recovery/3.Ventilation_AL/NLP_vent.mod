...
param specQ_people{Buildings} default 0.007; #;# specific average internal gains from people [kW/m2]{Buildings}
...
subject to Uenvbuilding{b in MediumTempBuildings}: Uenv[b]+mair*Cpair/3600=k_th[b]; # Uenv calculation for each building based on k_th and mass of air used
...
subject to Heat_Vent1 {t in Time}: Heat_Vent[t]=sum{b in MediumTempBuildings} ((mair/3600)*FloorArea[b]*Cpair*(Text_new[t]-Text[t])); #HEX heat load from one side;
	

subject to Heat_Vent2 {t in Time}: Heat_Vent[t]=sum{b in MediumTempBuildings} ((mair_out/3600)*FloorArea[b]*Cpair*(Tint-Trelease[t])); #HEX heat load from the other side;
...
subject to Area_Vent1 {t in Time}: Area_Vent*Uvent*DTLNVent[t]>=Heat_Vent[t]; #Area of ventilation HEX
	

subject to DTminVent1 {t in Time}: Trelease[t]>=DTminVent+Text[t]; #DTmin needed on one side of HEX
	

subject to DTminVent2 {t in Time}: Tint>=DTminVent+Text_new[t]; #DTmin needed on the other side of HEX 
...
subject to Flows{t in Time}: MassEPFL[t]*(EPFLMediumT-EPFLMediumOut)=Qheating[t]; #MCp of EPFL heating fluid calculation.
...
subject to Electricity1{t in Time}: E[t]+Qevap[t]=Qcond[t]; #the electricity consumed in the HP can be computed using the heat delivered and the heat extracted (Reference case)
	

subject to Electricity{t in Time}: E[t]*COP[t]=Qcond[t]; #the electricity consumed in the HP can be computed using the heat delivered and the COP (Reference case)
	

subject to COPerformance{t in Time}: COP[t]*(TLMCond-TLMEvap)=CarnotEff*(TLMCond); #the COP can be computed using the carnot efficiency and the logarithmic mean temperatures in the condensor and in the evaporator (Reference case)
	

subject to dTLMCondensor{t in Time}: TLMCond=(EPFLMediumOut-EPFLMediumT)/log((EPFLMediumOut+273)/(EPFLMediumT+273)); #the logarithmic mean temperature on the condenser, using inlet and outlet temperatures. Note: should be in K (Reference case)
	

subject to dTLMEvaporatorHP{t in Time}: TLMEvap=(THPhighin-THPhighout)/log((THPhighin+273)/(THPhighout+273)); #the logarithmic mean temperature can be computed using the inlet and outlet temperatures, Note: should be in K (Reference case)
	

## MEETING HEATING DEMAND, ELECTRICAL CONSUMPTION

subject to QEPFLausanne{t in Time}: Qheating[t]=Qcond[t]; #the heat demand of EPFL should be supplied by the the HP.
