...
subject to AreaHEDC{t in Time}: 
	AHEDC>=Qrad[t]/(UDC*(dTLMDC[t])); #the area of the heat recovery HE can be computed using the heat extracted from DC, the heat transfer coefficient and the logarithmic mean temperature difference 
...
subject to Freecooling1{t in Time}:
	Qfree[t]=MassDC*(TDCout[t]-Tret); #Free cooling from one side

subject to Freecooling2{t in Time}:
	Qfree[t]=Flow[t]*Cpwater*(THPin[t]-THPhighin); #Free cooling from the other side

subject to QEvaporator{t in Time}:
	Qevap[t]=Flow[t]*Cpwater*(THPin[t]-THPhighout); #water side of evaporator that takes flow from Free cooling HEX

subject to QCondensator{t in Time}:
	Qcond[t]=MassEPFL[t]*(EPFLMediumT-TRadin[t]); #EPFL side of condenser delivering heat to EFPL
	
subject to HeatBalanceDC{t in Time}:
	Qrad[t]+Qfree[t]=HeatDC; #makes sure all HeatDC is removed;
		
subject to Electricity1{t in Time}: 
	E[t]+Qevap[t]=Qcond[t]; #the electricity consumed in the HP can be computed using the heat delivered and the heat extracted

subject to Electricity{t in Time}:
	E[t]*COP[t]=Qcond[t]; #the electricity consumed in the HP can be computed using the heat delivered and the COP

subject to COPerformance{t in Time}:
	COP[t]*(TLMCond[t]-TLMEvapHP[t])=CarnotEff*(TLMCond[t]); #the COP can be computed using the carnot efficiency and the logarithmic mean temperatures in the condensor and in the evaporator

subject to dTLMCondensor{t in Time}:
	TLMCond[t]=(EPFLMediumT + TRadin[t])/2 + 273;
...
subject to dTLMEvaporator{t in Time}:
	TLMEvapHP[t]=(THPin[t] + THPhighout)/2 + 273;
...
subject to QEPFLausanne{t in Time}:
	Qheating[t]/top[t]=Qcond[t]+Qrad[t]; #the heat demand of EPFL should be the sum of the heat delivered by the 2 systems;
