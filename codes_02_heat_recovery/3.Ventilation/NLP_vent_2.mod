...
param mair 				:= 2.5/3600; #[m3/(m2 h)]*[h/s] -> [m/s]
param effvent           := 0.8; 	 #[-] ventilation efficiency -> assume 20% loss through vent
param Cpair 			:= 1.152;    #[kJ/(m3 K)]
param Uvent 			:= 0.025;    #[kW/(m2 K)] air-air HEX
...
var MassEPFL{Time} 	>= 0;     #[KJ/(s degC)] MCp of EPFL heating system 
...
subject to VariableHeatdemand {t in Time} : #Heat demand calculated as the sum of all buildings -> medium temperature
	#Text[t]<16 ==> Qheating[t] = sum{b in MediumTempBuildings} FloorArea[b]*(Uenv[b]*(Tint-Text[t])-mair*Cpair*(Tint-Text_new[t])-k_sun[b]*irradiation[t]-specQ_people[b]-share_q_e*specElec[b]) else Qheating[t] = 0;
