...
This first section aims to estimate the energy demand of the EPFL campus. The campus being divided into different buildings, with each its own thermal properties depending on many properties, among which wall material, window ratio, insulation, it is important to consider them individually in the analysis. Analyzing the buildings, the total yearly energy demand can be computed, considering the thermal gains from people and electronic appliances, as well as the gain from the sun and the external weather conditions. Finally, typical periods are regrouped into clusters according to temperature and radiation, enabling a better data management for the next sections.

## Buildings Energy Balance {#subsec:building-energy-balance}

To characterize the energy demand, the annual energy consumption of the buildings is needed, which is given in the following format:
...
Where $Heated surf$ is the building surface area in $[m^2]$, $Heat cons$ is the building annual heating consumption in $[kWh]$, $Elec cons$ is the building annual electricity consumption in $[kWh]$ and $Constr. zear$ is the building group identifier. All buildings in the group identifier 1 are heated in the temperature loop at $65$°C and all buildings in the group identifier 2 are heated in the temperature loop at $50$°C
...
The unknowns are $q_\text{people}(t)$, $q_\text{el}(t)$, $k_\text{th}$, and $k_\text{sun}$. To begin with, the occupancy profile of the buildings, the heat generation from people and the electricity demand will be derived, using the known annual demand.

## Hourly Profiles {#subsec:hourly-profiles}

The electricity demand profile is assumed to be constant when electrictiy is turned on, which is on weekdays from 7 AM to 9 PM. The values of the uniform electrictiy profile are calculated by dividing the specific electricity consumption of the buildings by the number of operating hours, i.e. 3654 hours per year.
...
The occupancy profile is assumed to be the same across all buildings. Following the SIA audit, the heat generated by people depends on the type of room considered. To every building one assigns a share of the total area with dedicated types of rooms, named $A_{share,i}$. Using this, it is possible to derive the specific heat gain by the people, determined by the equation @eq-people-heat-gain: 
$$
...
Notice that this is a constant value since the profiles $p_i(t)$ are supposed to be the same for all buildings. Now, $q_\text{el}(t)$ and $q_\text{people}(t)$ are known.

## Estimation of $k_\text{th}$ and $k_\text{sun}$ {#subsec:estimation-of-k-th-and-k-sun}

The two remaining unknowns are the thermal and solar constants, $k_\text{th} [\frac{W}{K \cdot m^2}]$ and $k_\text{sun} [-]$. Since the data available are the heating consumption $Q_\text{th,year}^+$, the equation @eq-building-balance has to be restricted accordingly, leading to a first equation: $$
Q_\text{th,year}= \sum Q_\text{th}^+(t)
$$ {#eq-heat-demand}

In short, the annual heating demand is the sum of all hourly demands. The hourly demands are considered as heating demands only if $Q_\text{th} > 0$.

The equation @eq-building-balance alone is not sufficient to solve the problem, but assuming a specific operating condition, it is possible to construct another one, i.e. when the heat demand $Q_\text{th}$ is zero. When the system is in this "no-load" condition, the temperature gradient is perfectly balanced by the people heat gains, the sun irradiance, and the electricity heat gains. The switch-off condition is assumed to be valid when sufficiently close to the cutoff temperature $T_\text{th}$. Taking an average of all of these values when the heating is turned off allows stating a second equation @eq-switch-off: $$
...
Combining equations @eq-heat-demand and @eq-switch-off allows constructing a system of two equations and two unknowns. However, since a sum of piecewise functions is involved in equation @eq-heat-demand, the system is nonlinear. The solutions will thus be estimated through numerical methods, e.g. a 2D Newton-Raphson algorithm.
...
Clustering via Machine Learning is now widely spread and lot of tools allow for an easy implementation. Using Sci-Kit, the weather data are clustered using unsepervised algorithms. Kmeans, Spectral clustering and agglomerate clustering are probed. A common metric used to asses of the relevancy of such clustering is the Sum of Squared Error (SSE). The visualisation of the clustering for $k=10$ clusters is shown in the figure (@fig-clusteringComp). The SSE is summarized in the table (@tbl-clusteringSSE).

*Note:* the data has been normalized to perform the clustering to improve perfomances, and reversed back to be plotted.
...
For the sake of completion, we mention that a Silhouette test has also been performed and returned an optimal $k=4$. However, the overall scores were all low, due to the metric's nature: since all the data points are lying in a region, data structure discovery is unlikely and clustering acts more as a way to split the space evenly. And since Silhouette score penalizes intercluster closeness, all scores drops since no clear clusters are underyling the distribution. In the end, the error on the variable of interest, $Q_{th}$, is almost divided by two from $k=4$ to $k=6$, and is thus still a valid choice.
...
This plot allows us to see that the resulting clustering visually seems to make sense. When reconstructing the clustered heat demands, we do observe 2 null demands: those are the clusters in the top right corner of our plot, which makes sense since those are the one with the highest irradiance and exterior temperature, inducing the lowest, here zero, heat demands.

```{python, results='hide', echo=F}
#| label: add-deviation

merged_df = pd.merge(weather, cluster, left_on='Cluster', right_index=True)
print(merged_df.head())
# calculate the deviation for each row
merged_df['Delta_Temp'] = abs(merged_df['Temp_x'] - merged_df['Temp_y'])
merged_df['Delta_Irr'] = abs(merged_df['Irr_x'] - merged_df['Irr_y'])

cluster = cluster.assign(Max_Delta_Temp =  merged_df.groupby('Cluster')['Delta_Temp'].max(),\
               Max_Delta_Irr =  merged_df.groupby('Cluster')['Delta_Irr'].max())
...
cluster.rename(columns={
    ...
    'Max_Delta_Temp': 'Max temp deviation [°C]',
    'Max_Delta_Irr': 'Max irr deviation [W/m²]',
}, inplace=True)
```

```{python, echo=F, eval = T, fig.cap='Summary of the clustering', label='tbl-cluster-summary'}
...
Computing the sum of the type A heat demands and the sum of the cluster heat loads allows to compute the relative error we've made on the total heat demand reconstruction, which is `r reticulate::py_eval("f'{clustering_error:.4}'")`%. The maximum intra cluster profile deviation for both temperature and irradiance is also reported in the @fig-cluster-summary.