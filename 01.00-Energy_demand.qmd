...
This first section aims to estimate the energy demand of the EPFL campus. The campus being divided into different buildings, with each its own thermal properties depending on the wall properties, window ratio, orientation, etc., it is important to consider them in the analysis. Analysing the buildings, the total yearly energy demand can be computed, considering the thermal gains from people and electronic appliances, as well as the gain from the sun and the external weather conditions. Finally, typical periods are regrouped into cluster according to temperature and radiation enabling a better data management for the next sections.
...
Clustering via Machine Learning is now widely spread and lot of tools allow for easy implementation. Using Sci-Kit, the weather data are clustered using unsepervised algorithms. Kmeans, Spectral clustering and agglomerate clustering are probed. A common metric used to asses of the relevancy of such clustering is the Sum of Squared Error (SSE). The visualisation of the clustering for $k=10$ clusters is shown in the figure (@fig-clusteringComp). The SSE is summarized in the table (@tbl-clusteringSSE).
