...
```{r task-2-case-1, out.width='100%', fig.align='center', fig.cap='Case 3: Air ventilation heat recovery'}
#| label: fig-task-2-case-3
knitr::include_graphics('Figures/task_2_case_3.png')
```

For this 2nd scenario, heat recovery is performed via air ventilation (HEX). This time the clustered weather data will be used(Temperature [°C] and Irradiation [$kW/m{^2}$]). 
Recall that 6 cluster points were defined for the weather data. In addition to this, in order to ensure the heating system is still working in extreme cases, a 7th cluster point is added. This cluster point is defined as the coldest day of the year with an irradiation of 0 [$kW/m{^2}$].
...
Thanks to the air-ventilation preheating, the heat demand will reduce according to the following equation:
...
All constraint equations from reference case are not repeated here but must also be considered as constraints.
...
```{r task-2-case-1, out.width='100%', fig.align='center', fig.cap='Case 4: Air ventilation with Heat pump integration'}
