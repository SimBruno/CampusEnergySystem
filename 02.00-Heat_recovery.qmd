...
In this section, EPFL's current heat supply system ans possible improvments are investigated. At first, the reference case is presented, where heat is provided to EPFL at $T=65°C$ (medium loop heating), using the lake water as cooling and a single heat pump to provide the necessary heating demand. Then in order to improve this system, 3 scenarios are assessed:
...
html_table_ref_data += f"<tr><td>Objective</td><td>{ref_TOTEX} CHF/yr</td></tr>\n"
...
Where $Q_{free}(t)$ and $Q_{rad}(t)$ are the heat loads exchanged in the free cooling heat exchanger and the heat exchanger respectively. The lifetime considered is $n=20$ years and the interest is $i=5\%$. Note that for this scenario, the Heat pump COP is not constant, since inlet and outlet temperatures of the condenser and evaporator are not constant. The problem becomes non-linear. It is possible to reduce the complexity by linearizing the log-mean temperatures for the Heat pump into mean temperatures:
...
html_table_dc_data += f"<tr><td>TOTEX</td><td>{DC_TOTEX} CHF/yr</td></tr>\n"
html_table_dc_data += f"<tr><td>OPEX</td><td>{DC_OPEX} CHF/yr</td></tr>\n"
html_table_dc_data += f"<tr><td>CAPEX</td><td>{DC_CAPEX} CHF/yr</td></tr>\n"
...
html_table_vent_data += f"<tr><td>TOTEX</td><td>{vent_TOTEX} CHF/yr</td></tr>\n"
html_table_vent_data += f"<tr><td>OPEX</td><td>{vent_OPEX} CHF/yr</td></tr>\n"
html_table_vent_data += f"<tr><td>CAPEX</td><td>{vent_CAPEX} CHF/yr</td></tr>\n"
...
html_table_vent_HP_data += f"<tr><td>TOTEX</td><td>{vent_HP_TOTEX} CHF/yr</td></tr>\n"
html_table_vent_HP_data += f"<tr><td>OPEX</td><td>{vent_HP_OPEX } CHF/yr</td></tr>\n"
html_table_vent_HP_data += f"<tr><td>CAPEX</td><td>{vent_HP_CAPEX} CHF/yr</td></tr>\n"
...
The cost reduction is `r reticulate::py_eval("f'{ref_TOTEX-vent_HP_TOTEX:.2f}'")` \[CHF/y\] and the payback time allows to breakeven after `r reticulate::py_eval("f'{vent_HP_payback:.2f}'")` years.

## Conclusion: which scenario enables a higher cost savings?

The results of all the optimizations are summarized in the @fig-summary-table, where the important economical indicator are reminded. We can see that, according to our calculation, the DC recovery and heat ventilation are yielding similar added CAPEX, but the latter has a better payback time thanks to its higher OPEX reduction. However, the clear winner is the last case, where the addition of a heat pump allows for the biggest overall cost reduction and the fastest payback time of all. The implementation of a ventilation heat exchanger coupled with a heat pump is thus the scenario we're advising economically.

```{python, eval = T, echo = F, results='hide'}
#| label: fig-summary-table
#| fig-cap: Optimization results summary
...
Html_file= open(os.path.join(os.getcwd(),"codes_02_heat_recovery","results","table_summary"),"w")
Html_file.write(html_table_summary)
Html_file.close()
...
data = {
    'Case': ['ref case', 'DC case', 'vent case', 'vent_HP case'],
    'OPEX [CHF/year]': [627362.05, 601755.22, 572749.68, 489240.35],
    'CAPEX [CHF/year]': [0, 12064.6, 12099.71, 26869.12],
    'TOTEX [CHF/year]': [627362.05, 613819.82, 584849.39, 516109.47]
}
df1 = pd.DataFrame(data)
 
fig = px.bar(df1, x="Case", 
             y=["OPEX [CHF/year]", "CAPEX [CHF/year]"], 
             title="Cost Visualisation for the 4 Different Cases")
fig = fig.update_layout(yaxis_title="Cost [CHF/year]")
