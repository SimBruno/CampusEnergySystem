...
\small Q_{evap}(t) = \dot{M}_{CP,Lake}(t) \cdot (T_{HP,high-in}-T_{HP,high-out})~\forall t\\
\small Q_{cond}(t) = \dot{M}_{CP,EPFL}(t) \cdot (T_{EPFL,medium}-T_{EPFL,medium-out})~\forall t\\
\small E(t) + Q_{evap}(t) = Q_{cond}(t) ~\forall t\\
\small E(t) = \dfrac{Q_{cond}(t)}{COP}~\forall t\\
\small \dot{M}_{CP,EPFL}(t)\cdot (T_{EPFL,medium}-T_{EPFL,medium-out}) = \dfrac{Q_{heating}(t)}{t_{op}(t)} ~\forall t\\
\small T_{LM,cond}=\dfrac{T_{EPFL,medium}-T_{EPFL,medium-out}}{\ln{\left( \dfrac{T_{EPFL,medium}}{T_{EPFL,medium-out}}\right)}}\\
\small T_{LM,evap}=\dfrac{T_{HP,high-in}-T_{HP,high-out}}{\ln{\left( \dfrac{T_{HP,high-in}}{T_{HP,high-out}}\right)}}\\
\small COP=\epsilon \dfrac{T_{LM,cond}}{(T_{LM,cond}-T_{LM,evap})}\\
\end{array}
$$

Where $\dot{M}_{CP,Lake}$ and $\dot{M}_{CP,EPFL}$ are the flow of the lake and the EPFL supply system respectively. Note that $E$ is the heat pump electricity in [kW], $Q_{cond}$ and $Q_{evap}$ are the heat power in [kW] flowing through the condenser and the evaporator respectively, $Q_{heating}(t)$ is the EPFL medium heat consumption. Data is clustered using the clusters produced before. $t_{op}$ is the operational time for each cluster. $T_{LM,cond}$, $T_{LM,evap}$ are the logariithmic mean temperatures at the condenser and the evaporator respectively.

Using AMPL, the problem is solved and the OPEX is: ...
