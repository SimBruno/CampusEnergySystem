...
In this section, EPFL's current heat supply system an possible improvments are investigated. At first, the reference case is presented, where heat is provided to EPFL at $T=65°C$, using the lake water as cooling and a single heat pump to provide the necessary heating demand. Then in order to improve this system, 3 scenarios are assessed:
...
DC_DTmin    = round(DC_data['DTmin'].values[0][0],2)
...
The cost reduction is `r reticulate::py_eval("f'{ref_TOTEX-vent_TOTEX:.2f}'")` [CHF/y] and the payback time allows to breakeven after `r reticulate::py_eval("f'{vent_payback:.2f}'")` years. The minimal DT for the ventilation HEX is `r reticulate::py_eval("f'{vent_DTminVent:.2f}'")` degrees, which is higher than for the DC recovery. This is expected since the calorific power of an Air-Air HEX is lower.
...
vent_HP_payback  = round(vent_HP_CAPEX*20/(vent_HP_OPEX-ref_OPEX),2) #capex is annualized over 20 years
