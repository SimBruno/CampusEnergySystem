...
# Read data from CSV
data_file = "clusters_data.csv" # assumes .csv is located in codes_01_energy_demand
...
##Build model for reference case
...
print(f'OPEX = {totalcost.get_values()}')
```
...
totalcost = ampl.getObjective("TCost")
...
```{python, eval = T, echo = F, results = 'show'}
#| label: ampl-vent
...
total_cost_value = ampl.getVariable('TC').value()
opex_value = ampl.getVariable('OPEX').value()
capex_value = ampl.getVariable('CAPEX').value()

# Create a Pandas DataFrame
vent_df = pd.DataFrame({
    'Variable': ['Total Cost', 'OPEX', 'CAPEX'],
    'Value': [total_cost_value, opex_value, capex_value]
})

print(vent_df)

html_vent = vent_df.to_html()
title_html = '<h2 style="text-align:center;">Test HTML</h2>'
html_vent_final = title_html +html_vent
HTML(html_vent_final)
...
# Print the values
print(f"OPEX: {opex_value}")
print(f"CAPEX: {capex_value}")
```
