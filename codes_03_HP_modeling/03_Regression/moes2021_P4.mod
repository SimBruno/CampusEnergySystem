...
param T_cond{Time} default 0; 		#[°C] condensation temperature of the hp 
param T_evap{Time} default 0; 		#[°C] evaporation temperature of the hp 
param T_ext {Time} default 0; 		#[°C] external temperature 
param T_hp_5{Time} default 0; 		#[°C] temperature in 5, see flowsheet
param T_EPFL_out{Time} default 0; 	#[°C] temperature return EPFL
param T_EPFL_in{Time} default 0; 	#[°C] temperature supply EPFL
param T_lake_in := 7;
param T_lake_out := 3;


param Q_cond{Time} default 0; 		#[kW] heat condensor
param Q_evap{Time} default 0; 		#[kW] heat evaporator
param W_comp1{Time} default 0;		#[kW] power of compressor 1
param W_comp2{Time} default 0 ; 	#[kW] power of compressor 1 
...
#################
param T_cond_max := T_cond[!!!];
param Q_cond_max := Q_cond[!!!];
param T_evap_max := T_evap[!!!];
param Q_evap_max := Q_evap[!!!];
param T_EPFL_in_max := T_EPFL_in[!!!];
param T_EPFL_out_max := T_EPFL_out[!!!];
param W_comp1_max := W_comp1[!!!];
param W_comp2_max := W_comp2[!!!];

#################
# !!!!! Fill here: maximum mass flow according to previously defined parameters (Q=mcp*dT)
#################
param Mcp := !!!; # close loop EPFL temperature liquid (with maximum heat load)
...
subject to CarnotFactor1{t in Time}:  #caculates the carnot factor for all time steps, avoid dividing by 0 in summer! 
	W_hp[t] >0 ==> c_factor1[t] = Q_cond[t]/ W_hp[t] / (TlnCond[t]+273.15)*(!!!-TlnCond[t]) else c_factor1[t] =0.001 ;
...
subject to TlnCond_constraint{t in Time}: #calculates the Log mean temperatrure of the epfl  temperature loop 
	(!!!) * log((!!!)/(T_EPFL_out[t]+273)) = (!!!-T_EPFL_out[t]);
